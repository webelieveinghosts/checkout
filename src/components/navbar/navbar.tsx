import { BadgeDollarSignIcon, BoxIcon, ChevronRight, ShoppingBasketIcon } from "lucide-react"
import { Logo } from "../logo/logo"

export const Navbar = ({ action = "details" }: { action: "details" | "payment" | "success" }) => {
    return (
                ...
                <div className="relative">
                    <ul className="relative flex w-full items-center justify-between space-x-2 sm:space-x-4">
                        ...
                        {action === "details" ? <>
                            ...
                            {action === "payment" ? <li className="flex items-center space-x-3 text-left sm:space-x-4">
                                <div className="flex h-6 w-6 items-center justify-center rounded-full bg-gray-600 text-xs font-semibold text-white ring ring-gray-600 ring-offset-2">
                                    <BadgeDollarSignIcon size={16} />
                                </div>
                                <span className="font-semibold text-gray-500">Pagamento</span>
                            </li> : <li className="flex items-center space-x-3 text-left sm:space-x-4">
                                <div className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-200 text-xs font-semibold text-emerald-700">
                                    <BadgeDollarSignIcon size={16} />
                                </div>
                                <span className="font-semibold text-gray-900">Pagamento</span>
                            </li>}
                        </>}
                    </ul>
                </div>
